...
struct hash_table_v1 *hash_table_v1_create()
{
	...
	int err = pthread_mutex_init(&hash_table->v1_lock, NULL);
    handle_pthread_error(err, "pthread_mutex_init");
...
void hash_table_v1_add_entry(struct hash_table_v1 *hash_table,
                             ...
                             uint32_t value)
{
	int err = pthread_mutex_lock(&hash_table->v1_lock);
    handle_pthread_error(err, "pthread_mutex_lock");
	struct hash_table_entry *hash_table_entry = get_hash_table_entry(hash_table, key);
	struct list_head *list_head = &hash_table_entry->list_head;
	struct list_entry *list_entry = get_list_entry(hash_table, key, list_head);
	...
	if (list_entry != NULL) {
		...
		err = pthread_mutex_unlock(&hash_table->v1_lock);
    	handle_pthread_error(err, "pthread_mutex_unlock");
		return;
	}
...
void hash_table_v1_destroy(struct hash_table_v1 *hash_table)
{
	...
	int err = pthread_mutex_init(&hash_table->v1_lock, NULL);
    handle_pthread_error(err, "pthread_mutex_destroy");
	free(hash_table);
}
